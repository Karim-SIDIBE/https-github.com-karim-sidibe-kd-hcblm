...
import { z } from "zod";
import { BankError, approveBankQuestion, createBankQuestion, deleteBankQuestion, distinctSubAreas, importFromCourse, listBankQuestions, randomBankQuestions, updateBankQuestion } from "./bank.service.js";
...
export async function bankRoutes(app: FastifyInstance) {
  ...
  app.patch("/bank/questions/:id", { preHandler: guard("course:update") }, async (req, reply) => {
    const { id } = z.object({ id: z.string() }).parse(req.params);
    const body = z.object({ question: z.unknown().optional(), subArea: z.string().optional(), level: z.string().optional() }).parse(req.body);
    try { return { data: await updateBankQuestion(id, body) }; } catch (err) { return handle(reply, err); }
  });
