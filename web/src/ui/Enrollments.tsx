...
export function Enrollments() {
  ...
  if (error && !list) {
    ...
    return (
      <div>
        <h1>{t("enr.myCourses")}</h1>
        <p className="banner offline">⚠️ {error}</p>
        {known.length > 0 && (
          <div className="stack">
            <p className="muted">{t("enr.offlineAvailable")}</p>
            {known.map((id) => <button key={id} className="block secondary" onClick={() => open(id)}>{t("enr.open", { id: id.slice(0, 8) })}</button>)}
          </div>
        )}
      </div>
    );
  ...
  if (!list) {
    return <div><h1>{t("enr.myCourses")}</h1>{[0, 1].map((i) => <div key={i} className="card"><div className="skeleton line" style={{ width: "60%" }} /><div className="skeleton card" /></div>)}</div>;
  }
  ...
  return (
    <div className="stack">
      <div>
        <h1>{t("enr.myCourses")}</h1>
        {list.length === 0
          ? <p className="muted">{t("enr.none")}</p>
          : (
            <div className="stack">
              {list.map((e) => (
                <article key={e.id} className="card tappable" onClick={() => open(e.id)} role="button" tabIndex={0}
                  ...
                  <div className="row between">
                    ...
                    <span className={`chip ${e.status === "CERTIFIED" ? "ok" : ""}`}>{e.status === "CERTIFIED" ? t("enr.certified") : levelLabel(e.course.level)}</span>
                  </div>
                  <div className="progress" style={{ margin: "10px 0 6px" }}><span style={{ width: `${e.progressPercent}%` }} /></div>
                  <p className="muted" style={{ margin: 0 }}>{t("enr.blocks", { done: e.blocksCompleted, total: e.blocksTotal, pct: e.progressPercent })}</p>
                </article>
              ))}
            </div>
          )}
      ...
      {available.length > 0 && (
        <div>
          <h2 style={{ marginBottom: 4 }}>{t("enr.available")}</h2>
          <p className="muted" style={{ marginTop: 0 }}>{t("enr.enrollOneClick")}</p>
