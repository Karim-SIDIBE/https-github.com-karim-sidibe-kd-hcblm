...
export function Deliverable({ eid, block, itemKey }: { eid: string; block: number; itemKey: string }) {
  ...
  const spec = useMemo(() => {
    ...
    if (itemKey === "field") {
      ...
      return fa ? { kind: "field" as const, eyebrow: "Application terrain — obligatoire", title: "Application terrain", brief: fa.brief, min: fa.minChars ?? 200, unit: "caractères", itemType: "FIELD_APPLICATION" } : null;
    }
    const entry = (blk?.payload?.journal?.entries ?? []).find((e: any) => `J+${e.day}` === itemKey);
    return entry ? { kind: "journal" as const, eyebrow: "Journal de bord", title: `Journal — Jour ${itemKey}`, brief: entry.prompt, min: entry.minWords ?? 50, unit: "mots", itemType: "JOURNAL_ENTRY" } : null;
  ...
  if (!spec) return <div className="stack"><Back /><p className="banner offline">Élément introuvable.</p></div>;
    ...
    <div className="stack">
      ...
      <div className="hf-card hf-card--stripe-orange stack">
        ...
        <div className="hf-textwrap">
          <textarea className="hf-field" value={text} onChange={(e) => setText(e.target.value)} placeholder="Votre réponse, ancrée dans votre situation réelle…" style={{ minHeight: 180 }}
            onFocus={(e) => setTimeout(() => e.target.scrollIntoView({ block: "center", behavior: "smooth" }), 200)} />
          <span className="hf-count" style={{ color: ok ? "var(--brand-declick)" : undefined }}>{count} / {spec.min} {spec.unit}</span>
        </div>
        <button className="hf-btn hf-btn--primary hf-btn--block" disabled={busy || !ok} onClick={submit}>{busy ? "…" : "Soumettre →"}</button>
      </div>
    </div>
  );
}
